...
1. Open the Unraid Drive app: since build 7 it nudges every location whenever it comes to the foreground; since build 34 the first launch and every update also **check the connection and rebuild every location automatically** (the server page shows *Files location: checked and rebuilt automatically* with the date, or *Automatic check pending* with a *Check now* button when the gateway was unreachable). **Test connection** nudges the location in its last check. Since build 8 the extension also re-reads the list of shares on every sync pass, so a share you unmapped from the container disappears from Files (together with anything cached below it) instead of producing endless "Error" retries. Since build 10 the app also rebuilds each location once per installation: after a reinstall (or a restore from iCloud) iOS keeps the old cached tree for the same server while the extension's own index is gone, so the two disagree and every share shows "Error ↑" with the location paused. The rebuild removes the location and registers it again. You can trigger it yourself from the server page with **Rebuild the Files location** (files created in Files but never uploaded are lost).

"Sync paused" at the bottom of a folder means that some item in this location cannot be uploaded, or that the gateway was unreachable the last time iOS tried; fix or delete that item and the status goes back to "Synced" (the location is rebuilt automatically at the next app update; *Check now* on the server page repeats the check).
