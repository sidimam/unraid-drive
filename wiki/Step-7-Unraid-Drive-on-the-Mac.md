...
- **⚙ gear** — Preferences (the main window), Offline files (space used on this Mac per server, *Free up space*), Error list, About, Help, Send feedback, **Launch at login**, **Menu bar only (hide Dock icon)**, Quit.

*Menu bar only* turns Unraid Drive into a menu-bar app: no Dock icon, the panel is the entry point and *Open Unraid Drive* brings the window back.
