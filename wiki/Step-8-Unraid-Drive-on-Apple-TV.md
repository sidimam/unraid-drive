...
Since build 36 selecting a server opens its shares directly; the **gear** in the header (at the share root) leads to *Dashboard* (system, array, and the unraid-gateway container with version and state), *Shares to show* and *Remove this server from the TV*.
