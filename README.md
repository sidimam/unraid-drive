...
`scripts/release.sh` archives, exports and uploads the iOS, visionOS, macOS and tvOS builds to TestFlight (`PLATFORMS="macOS tvOS"` to restrict); `scripts/make_dmg.sh` produces the Developer ID DMG for GitHub Releases and the Homebrew cask (needs `ASC_ISSUER` in the environment and the App Store Connect API key in `~/.appstoreconnect/private_keys/`). Both keep archives, intermediates and exports outside the repository, in `DD_ROOT` (default `~/Library/Caches/UnraidDrive-build`): a checkout inside Google Drive or another syncing folder gets extended attributes on the build products and CodeSign refuses them. `AppStore/store_meta.py` holds the localized App Store listing texts. `scripts/asc_submit.py` applies them and submits the versions for review through the App Store Connect API (ids of the day at the top of the file).
